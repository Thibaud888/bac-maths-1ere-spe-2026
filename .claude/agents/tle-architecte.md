...
- Facultatif : le rapport de fréquences des annales (sortie de `scripts/frequences-annales.mjs`,
  fourni par l'orchestrateur s'il existe), une consigne de Thibaud (ordre de la classe…).
...
1. **Lis**, dans cet ordre :
   ...
   - `content/terminale/<matiere>/annales.json` s'il existe ;
...
3. **Découpe en 3 à 7 notions.** Une notion = ce qu'un élève apprend en une séance (20 à 40 min
...
4. **Priorité de chaque notion** (charte § 5.1) :
   - avec rapport d'annales : applique les repères (≥ 50 % → 3, 20-50 % → 2, < 20 % → 1),
     `priorisation: "annales"`, `pourquoi` avec le chiffre (« Tombé dans 31 sujets sur 38
     depuis 2022 ») ;
   - sans : reprends l'estimation du chantier, `priorisation: "estimation"`, `pourquoi` qui
     dit ce qui fonde l'estimation, sans chiffre inventé ;
...
8. **Écris** `content/terminale/<matiere>/chapitres/<slug>/meta.json` et `notions.json`.
...
- ❌ Un chiffre de fréquence sans rapport d'annales ; une formulation « du bac » inventée.
