...
export default function BacBlancRunner({ paper, onClose }: Props) {
  ...
  return (
    <div className="space-y-5">
      <header className="rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-5 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            {paper.subtitle && (
              <p className="mt-1 text-sm text-slate-600 dark:text-slate-400">{paper.subtitle}</p>
            )}
        ...
        <div className="mt-4 flex gap-1 rounded-lg bg-slate-100 dark:bg-slate-700 p-1">
          <button
            type="button"
            onClick={() => {
              setTab('part1');
